import { IPhysicsManager } from '../design';
import {
  BoundingBox,
  BoundingSphere,
  Ray,
...
export class LitePhysicsManager implements IPhysicsManager {
  ...
  private _currentEvents: DisorderedArray<TriggerEvent> =
    new DisorderedArray<TriggerEvent>();
  ...
  private static _updateWorldBox(
    boxCollider: LiteBoxColliderShape,
    out: BoundingBox
  ): void {
  ...
  private static _upWorldSphere(
    sphereCollider: LiteSphereColliderShape,
    out: BoundingSphere
  ): void {
    Vector3.transformCoordinate(
      sphereCollider._transform.position,
      sphereCollider._transform.worldMatrix,
      out.center
    );
  ...
  private _getTrigger(index1: number, index2: number): TriggerEvent {
    const event = this._eventPool.length
      ? this._eventPool.pop()
      : new TriggerEvent(index1, index2);
      ...
      if (myShape instanceof LiteBoxColliderShape) {
        ...
        for (let j = 0, len = colliders.length; j < len; j++) {
          ...
          for (let k = 0, len = colliderShape.length; k < len; k++) {
            ...
            const event =
              index1 < index2
                ? this._eventMap[index1][index2]
                : this._eventMap[index2][index1];
            ...
            if (shape != myShape && this._boxCollision(shape)) {
              if (event === undefined) {
                const event =
                  index1 < index2
                    ? this._getTrigger(index1, index2)
                    : this._getTrigger(index2, index1);
