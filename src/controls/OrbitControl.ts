...
export class OrbitControl extends Script {
  ...
  onDisable(): void {
    ...
    this.mainElement.removeEventListener(
      this.mouseUpEvents[0].type,
      this.mouseUpEvents[0].listener,
      false
    );
    element.removeEventListener(
      this.mouseUpEvents[1].type,
      this.mouseUpEvents[1].listener,
      false
    );
  }
  ...
  pan(deltaX: number, deltaY: number) {
    ...
    this.panLeft(
      -2 * deltaX * (targetDistance / this.mainElement.clientHeight),
      this.camera.transform.worldMatrix
    );
    this.panUp(
      2 * deltaY * (targetDistance / this.mainElement.clientHeight),
      this.camera.transform.worldMatrix
    );
  }
  ...
  handleMouseMoveRotate(event) {
    ...
    this.rotateLeft(
      2 *
        Math.PI *
        (this._rotateDelta.x / this.mainElement.clientWidth) *
        this.rotateSpeed
    );
    this.rotateUp(
      2 *
        Math.PI *
        (this._rotateDelta.y / this.mainElement.clientHeight) *
        this.rotateSpeed
    );
  ...
  handleTouchMoveRotate(event: TouchEvent) {
    ...
    this.rotateLeft(
      ((2 * Math.PI * this._rotateDelta.x) / this.mainElement.clientWidth) *
        this.rotateSpeed
    );
    this.rotateUp(
      ((2 * Math.PI * this._rotateDelta.y) / this.mainElement.clientHeight) *
        this.rotateSpeed
    );
