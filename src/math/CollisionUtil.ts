...
export class CollisionUtil {
   ...
   */
  static intersectsPlaneAndPoint(
    plane: Plane,
    point: Vector3
   ...
   */
  static intersectsFrustumAndBox(
  ...
  static frustumContainsBox(
    ...
    for (let i = 0; i < 6; ++i) {
      ...
      if (
        CollisionUtil.intersectsPlaneAndPoint(plane, back) ===
      ...
      if (
        CollisionUtil.intersectsPlaneAndPoint(plane, front) ===
        PlaneIntersectionType.Front
      ) {
  ...
  static frustumContainsSphere(
    ...
    for (let i = 0; i < 6; ++i) {
      const plane = frustum.getPlane(i);
      const intersectionType = CollisionUtil.intersectsPlaneAndSphere(
        plane,
        sphere
      );
